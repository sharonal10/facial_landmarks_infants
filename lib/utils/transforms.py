...
def crop(img, center, scale, output_size, rot=0):
    ...
    if sf < 2:
        ...
        if new_size < 2:
            ...
            img = np.resize(img, [new_ht, new_wd]) #scipy.misc.imresize(img, [new_ht, new_wd])  # (0-1)-->(0-255)
    ...
    new_img = img = np.resize(img, output_size) #scipy.misc.imresize(new_img, output_size)
    return new_img
